...
export class Response<TSession, TI18n extends I18N> extends ServerResponse {
	...
	setCookie(
		name: string,
		value: string,
		options?: CookieSerializeOptions & { signed?: boolean }
	) {
	...
	async render(path: string, data?: Record<string, any>) {
		if (this.i18n == null)
			throw new GError(
				ErrorCodes.MISSING_I18N,
				'res.render>> Expected i18n'
			);
		var html = await this.app._render(
			this.i18n.locale,
			path,
			this.data,
			data
	...
	xml(data: any) {
		data = XMLConverter.js2xml(data, {
			spaces: this.app.options.pretty ? '\t' : 0
		});
	...
	sendFile(filePath: string | Buffer, options: SendFileOptions) {
		if (typeof filePath === 'string')
			return new Promise((res, rej) => {
				...
				this.setHeader(
					...
					ContentDisposition(fileName, {
						type: options.inline === false ? 'attachment' : 'inline'
					})
				);
				// send
				var streaming = false;
				...
				file.on('directory', function () {
					rej(
						new GError(
							ErrorCodes.EXPECTED_FILE_GOT_DIRECTORY,
							`Expected file not directory at: ${filePath}`,
							filePath
						)
					);
				});
				...
				file.on('error', function (err) {
					if (err == null)
						...
						err = new GError(
							ErrorCodes.SEND_FILE_ERR,
							`Error when sending file: ${filePath}`,
							err
						);
					rej(err);
				});
				OnFinishedLib(this, function (err) {
					if (err) rej(err);
						...
						rej(
							new GError(
								ErrorCodes.SEND_FILE_ABORTED,
								`File sending aborted: ${filePath}`
							)
						);
					else res(true);
				});
			...
			// Content type
			this.contentType ??=
				(fileName && mimeTypeLookup(fileName)) || undefined;
			// Content disposition
			this.setHeader(
				'Content-Disposition',
			...
			if (maxAge == null) {
				...
				if (typeof maxAge === 'string')
					maxAge = (ParseMilliseconds(maxAge) / 1000) >> 0;
	...
	send(data: string | Buffer | Record<string, any>, contentType?: string) {
		return new Promise((resolve, reject) => {
			...
			if (!this.hasHeader('etag'))
				this.setHeader(
					'etag',
					(options.etag as Function)(data as Buffer)
				);
			//* freshness
			...
			if (statusCode === 304 || statusCode === 204) {
				...
				if (req.method === 'HEAD') return super.end(resolve);
				else {
					//@ts-ignore

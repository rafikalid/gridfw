...
export class Request<TSession, TI18n extends I18N> extends IncomingMessage {
	...
	/** Client IP */
	get ip() {
		return ProxyAddr(this, this.app.options.trustProxy);
	}
	...
	async setLocale(locale?: string) {
		var app = this.app;
		if (locale == null) {
			locale = (this.acceptsLanguages(Array.from(app.locales)) ||
				app.defaultLocale) as string;
		}
		...
		if (this._uploading == null) {
			if (this._uploadBuffer != null)
				throw new GError(
					...
					'Could not call "upload" and "uploadBuffer" with the same request!'
				);
...
export interface RequestParams<T> extends BasicMap<T> {
	/** Get parsed & resolved value or param */
	resolve(key: string): Promise<any>;
}

/** Cookie map */
export interface CookieMap extends BasicMap<string> {
	/** Get Signed value of cookie */
	getSigned(key: string): string | undefined;
}

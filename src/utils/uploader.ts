...
export function upload(
	...
	return new Promise((resolve: (data: UploadResult) => void, reject) => {
		...
		//* Limit body size
		var bodySize: string | number | undefined =
			req.headers['content-length'];
		if (bodySize) {
			bodySize = Bytes(bodySize);
			if (bodySize > limits.size)
				throw new GError(
					ErrorCodes.UPLOAD_MAX_BODY_SIZE,
					`data size: ${Bytes(bodySize)}. Max size: ${Bytes(
						limits.size
					)}`
				);
		}
		...
		if (options.contentType != null) {
			...
			if (Array.isArray(type)) {
				...
				if (found === false)
					throw new GError(
						ErrorCodes.UPLOAD_CONTENT_TYPE,
				...
				if (type !== contentType)
					throw new GError(
						ErrorCodes.UPLOAD_CONTENT_TYPE,
						`Expected: ${type}. Received: ${contentType}`
					);
			}
		}
		//* Upload progress
...
function uploadFormData(
	...
	const onFile = createFileUploadWrapper(
		req,
		options,
		tempPaths,
		errorHandler
	...
	busboy
		...
		.on('partsLimit', function () {
			errorHandler(new GError(ErrorCodes.UPLOAD_ERROR, 'partsLimit'));
		})
		.on('filesLimit', function () {
			errorHandler(new GError(ErrorCodes.UPLOAD_ERROR, 'filesLimit'));
		})
		.on('fieldsLimit', function () {
			errorHandler(new GError(ErrorCodes.UPLOAD_ERROR, 'fieldsLimit'));
		})
		.on(
			'field',
			function (
...
function uploadRawData(
	...
	try {
		...
		var data: any;
		var onFile = createFileUploadWrapper(
			req,
			options,
			tempPaths,
			_errorhandler
...
function createFileUploadWrapper(
	req: Request<any, any>,
	options: FileUploadOptions,
	tempPaths: string[],
	...
	return async function (
		...
		try {
			...
			//* Switch result
			if (result == null) {
				filePath = await _createTempFile(
					options.tempDir,
					result,
					req.now
				);
				...
				switch (result.charAt(0)) {
					...
					case '.':
						// Create temp file
						filePath = result = await _createTempFile(
							options.tempDir,
							result,
							req.now
						);
...
/** Get request body (for parsing or as buffer) */
export async function getBodyCb(
	req: Request<any, any>,
	maxSize: number | string | undefined,
	cb: (err: any, buffer: Buffer | string) => void
) {
	var stream: Readable | undefined;
	stream = createStream(req);
	RawBody(
...
/** Upload JSON */
function uploadJsonXml(
	req: Request<any, any>,
	options: FileUploadOptions,
	limits: UploadLimitsParsed,
	appOptions: Omit<ParsedOptions, 'baseURL'>,
	parser: (data: string) => Record<string, any>,
	...
	if (options.parse === true) {
		getBodyCb(req, limits.size, function (err, data) {
			if (err)
				reject(
					new GError(ErrorCodes.UPLOAD_ERROR, 'Upload failed', err)
				);
			else {
				if (typeof data !== 'string')
					data = data.toString(appOptions.encoding);
				resolve(
					new UploadResult(data.length === 0 ? {} : parser(data), [])
				);
			}
		});
	} else {
